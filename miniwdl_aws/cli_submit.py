...
def miniwdl_submit_awsbatch(argv):
    ...
    workflow_container_props = None
    if args.filesystem == "efs":
    ...
    # For FSx Lustre filesystem
        ...
        workflow_container_props = {
            ...
            "mountPoints": [{"containerPath": args.mount, "sourceVolume": "fsx"}],
            "image": args.image,
            "command": miniwdl_run_cmd,
            "environment": environment,
        }
...
def parse_args_and_env(argv):
    ...
    group.add_argument(
        "--filesystem",
        help="The filesystem option where workflow jobs mount to (either efs or fsx)",
    )
    group.add_argument(
        "--fsid",
    ...
    if not args.filesystem:
        print(
            "--filesystem required for which filesystem option being chosen",
            file=sys.stderr,
        )
        sys.exit(1)
    if args.filesystem == 'efs':
        args.fsap = args.fsap if args.fsap else os.environ.get("MINIWDL__AWS__FSAP", "")
    elif args.filesystem == 'fsx':
        args.fsxid = args.fsxid if args.fsxid else os.environ.get("MINIWDL__AWS__FS", "")
    else:
        print(
            "--filesystem invalid filesystem option",
            file=sys.stderr,
        )
        sys.exit(1)
    args.workflow_queue = (
    ...
    if args.filesystem == 'efs':
        ...
        if not (args.fsid.startswith("fs-") and args.workflow_queue and args.task_queue):
            print(
                "--fsxid, --workflow-queue, and --task-queue all required (or environment variables MINIWDL__AWS__FSID, MINIWDL__AWS__WORKFLOW_QUEUE, MINIWDL__AWS__TASK_QUEUE)",
                file=sys.stderr,
            )
            sys.exit(1)
